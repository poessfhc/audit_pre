        ...
        >
          ...
          <el-table-column align="left">
            <template slot-scope="scope">
              <el-button size="mini" @click="handleEdit(scope.$index, scope.row)">工程实施</el-button>
            </template>
          </el-table-column>
        </el-table>
        <implementDialog
          :implementDialogVisible="implementDialogVisible"
          :implementDialogInfo="implementDialogInfo"
          :projectId="projectId"
          @update:implementDialogVisible="implementDialogVisibles"
        ></implementDialog>
        <el-pagination
          class="pagination"
...
import implementDialog from "@/components/dialog/implementDialog";
export default {
  inject: ["reload"],
  components: {
    implementDialog
  ...
  data() {
    return {
      //控制弹窗 显示
      implementDialogVisible: false, //点击查看按钮  这条数据详细信息
      implementDialogInfo: {},
      total: 0,
      currentPage: 1, //初始页
      pagesize: 10, //    每页的数据
  ...
  methods: {
    ...
    handleEdit(index, row) {
      ...
      this.implementDialogInfo = row
    },
    implementDialogVisibles(v) {
      this.implementDialogVisible = v;
    }
  }
};
</script>
